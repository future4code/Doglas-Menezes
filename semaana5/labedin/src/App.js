import React from 'react';
import './App.css';
import CardGrande from './components/CardGrande/CardGrande';
import CardPequeno from './components/CardPequeno/CardPequeno';
import ImagemButton from './components/ImagemButton/ImagemButton';


function App() {
  return (
    <div className="App">
      <div className="page-section-container">
        <h2>Dados pessoais</h2>
        <CardGrande 
          imagem="https://scontent.fpoa13-1.fna.fbcdn.net/v/t1.6435-9/41930294_1351708311630353_889848504254464_n.jpg?_nc_cat=110&ccb=1-3&_nc_sid=84a396&_nc_ohc=NxDUTXiFhrYAX89LsFk&_nc_ht=scontent.fpoa13-1.fna&oh=849f6f3511dafb6df4cd06283012ae02&oe=60F3580F" 
          nome="Doglas Viegas" 
          descricao="Olá, Sou Doglas Viegas to fazendo o curso Dev.Full stack na labenu, descobri como é bom aaprender e se desenvolver na área que você gosta. pretendo me especializar como Front-end!! "
        />

        
        
        <ImagemButton 
          imagem="https://image.flaticon.com/icons/png/512/117/117472.png" 
          texto="Ver mais"
        />
      </div>
      <div className="page-section-container">
        <h2>Informações pessoais</h2>
        <CardPequeno 
          imagem="https://www.tijucas.sc.leg.br/imagens/email2icon.png/image" 
          nome="Email" 
          descricao="Doglas.menezes@hotmail.com" 
        />
       
       
         <CardPequeno 
          imagem="https://e7.pngegg.com/pngimages/218/575/png-clipart-computer-icons-symbol-independent-travel-blue-logo.png" 
          nome="Meu endereço" 
          descricao="Av.Protasio alves,4507-Morro santana,Porto Alegre-RS" 
        />
       </div>
      <div className="page-section-container">
        <h2>Experiências Profissionais</h2>
        <CardGrande 
          imagem="https://realizarpropositosetalentos.files.wordpress.com/2017/08/align-personal-and-professional.jpg" 
          nome="Minha vida Profissional" 
          descricao="Ainda não possuo experiências na área de Desenvolvedor, mas já exerci cargo de lidereraça como Maitre na gastronomia. para meu desenvolvimento pessoal isso foi extremamente importante e agora busco uma transição de carreira." 
        />
        
        <CardGrande 
          imagem="data:image/png;base64," 
          nome=" Oque ja aprendi na Labenu?" 
          descricao="Estou terminando o modulo 1 e ja aprendemos HTML,CSS,JavaScript. Executamos alguns trabalhos com essas aplicações e agora estamos começanado a programar com REACT oque abriu muitas possibilidades de aplicações novas! " 
        />
      </div>

      <div className="page-section-container">
        <h2>Minhas redes sociais</h2>
        <ImagemButton 
          imagem="https://d2v9ipibika81v.cloudfront.net/uploads/sites/261/2017/01/facebook-logo-3.png" 
          texto="Facebook" 
        />        

        <ImagemButton 
          imagem="https://logodownload.org/wp-content/uploads/2014/09/twitter-logo-1-1.png" 
          texto="Twitter" 
        /> 
                   
      </div>
    </div>
  );
}

export default App;
